...
## Hardened bazterpro fork

This public fork remains compatible with the HetrixTools v2 metrics endpoint,
but its executable supply chain is isolated from the upstream repository.

- Downloads are pinned to the `secure-v2.4.1-1` release tag in `bazterpro/agent`.
- The installer and updater verify the SHA-256 of the agent and default config.
- Arbitrary update branches are rejected.
- TLS certificate verification is mandatory.
- systemd is preferred over cron and applies a restrictive sandbox.
- Runtime can be either `root` (full SMART/RAID/NVMe visibility) or the
  unprivileged `hetrixtools` user.
- Running-process collection remains available and is controlled by
  `RunningProcesses` exactly as in the upstream agent.
- No workflow automatically synchronizes or merges upstream changes.

### Verified installation

Download the installer from the pinned tag and compare it with
`SHA256SUMS` before running it. Use the installation arguments generated by
the HetrixTools dashboard. The second argument selects the runtime user:
`1` for `root`, or `0` for `hetrixtools`.

```bash
wget https://raw.githubusercontent.com/bazterpro/agent/secure-v2.4.1-1/hetrixtools_install.sh
echo "4c44d6a71ec1148a1d37fa3d5d5585de9158f3b829ab0c33b1a631d0680be87d  hetrixtools_install.sh" | sha256sum -c -
sudo bash hetrixtools_install.sh YOUR_HETRIXTOOLS_INSTALL_ARGUMENTS
```

See [SECURITY.md](SECURITY.md) for the trust model and update procedure.
