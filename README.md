...
- Downloads are pinned to the `full-monitoring-v2.4.1-1` release tag in `bazterpro/agent`.
...
- The agent intentionally runs as `root` without a systemd sandbox so every
  supported metric remains available.
- SMART, RAID, NVMe and running-process collection are forced on.
...
the HetrixTools dashboard. This fork ignores the runtime-user selection and
always runs the monitoring process as `root`.

```bash
wget https://raw.githubusercontent.com/bazterpro/agent/full-monitoring-v2.4.1-1/hetrixtools_install.sh
echo "d7b4e9c69837d6f2799f657c98d89858390c93313fcb9558e86c8e29bfd84359  hetrixtools_install.sh" | sha256sum -c -
